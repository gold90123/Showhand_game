    ...
    public void run() {
        while (true) {
            try {
                ...
                outstream.writeLong(Client1_OwnCard.my_bet); //回合一開始就傳自己的本金(server 199行)
                ...
                // 接收對手的第一張明牌
                card = instream.readUTF();
                Client1_EnemyCard.owncardlist.add(card);
                ...
                card_string = card_string.replaceFirst("000", Client1_OwnCard.owncardlist.get(1));
                System.out.println("Your cards: " + card_string);
                long score = score_counting(card_string);
                System.out.println("score is: " + score);
                outstream.writeLong(score);

                Client1_EnemyCard.enemy_bet = instream.readLong(); //收對手的原始賭金(server 261行)
                ...
                for(int card_count = 2; card_count < 6; card_count++) {
                    ...
                    switch (smaller_or_bigger) {
                        case 0:
                            ...
                            if(find_bet[4].equalsIgnoreCase("raise")) {//如果對手raise
                                ...
                                Client1_EnemyCard.enemy_bet -= opponent_bets;
                            }
                            // 讀取 client 的動作
                            ...
                            if(find_bet[4].equalsIgnoreCase("showhand!!")) {
                                opponent_bets = Math.min(Client1_OwnCard.my_bet, Client1_EnemyCard.enemy_bet);
                            }
                    ...
                    if (User_input.equalsIgnoreCase("raise")) { // 選擇了加注
                        ...
                        Client1_OwnCard.my_bet -= bet; // 扣掉自己的錢
                        ...
                        long available_bets = Math.min(Client1_OwnCard.my_bet, Client1_EnemyCard.enemy_bet);
                        Client1_OwnCard.my_bet -= available_bets;
